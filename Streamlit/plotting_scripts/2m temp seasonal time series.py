...
#     plot_ecmwf_data(ecmwf_file, var='t2m', plot_type='seasonal')
...
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd # For pd.to_datetime if needed
...
def plot_2m_temp_seasonal_cycle(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Create a seasonal cycle plot (monthly averages over the year) for 2m temperature.
    ...
    try:
        ...
        # Ensure the time coordinate is a dimension of the data variable
        ...
        # Access .dt accessor via the time coordinate in the dataset `ds`, not directly on `data_var` if `dim_to_use_for_time` is different from `actual_time_coord_name`
        # However, `groupby(ds[actual_time_coord_name].dt.month)` should work as `ds[actual_time_coord_name]` is the 1D time coordinate array.
        ...
        if lat_coord_name not in data_var.dims or lon_coord_name not in data_var.dims:
            print(f"Error: Could not determine latitude/longitude coordinate names. Dims: {data_var.dims}", file=sys.stderr)
            sys.exit(1)
        ...
        # For temperature, convert to Celsius if it appears to be in Kelvin
        if plot_data.mean(skipna=True).item() > 100:  # Heuristic for Kelvin
            plot_data = plot_data - 273.15  # Convert to Celsius
            display_units = '°C'
            print(f"Data for '{nc_variable_name}' appears to be in Kelvin, converted to Celsius for seasonal cycle plot.", file=sys.stdout)
        else:
            original_units = data_var.attrs.get('units', 'K') # Get units from original data_var
        ...
        # Select color for t2m
        plot_color = sns.color_palette("viridis", 5)[4] # Index 4 for 't2m'
        
        # Ensure month coordinate exists and is 1-12 for plotting
        if 'month' not in plot_data.coords:
            # If 'month' is not a coordinate after groupby (e.g., it's an index level), create it
            plot_data = plot_data.rename({'month_of_year': 'month'}) # common xarray rename after groupby
            if 'month' not in plot_data.coords: # If still not there, try to assign from unique values
                unique_months = np.unique(ds[actual_time_coord_name].dt.month)
                if len(unique_months) == len(plot_data):
                    plot_data = plot_data.assign_coords(month=unique_months)
                else:
                    print("Warning: Could not reliably assign month coordinate for plotting seasonal cycle. X-axis might be incorrect.", file=sys.stderr)
                    # Fallback to simple range if month coord is problematic
                    plot_data.coords['month'] = np.arange(1, len(plot_data) + 1)


        plot_data.plot(marker='o', color=plot_color)
       
        plt.title(f'2m Temperature Seasonal Cycle (Area Average)', fontsize=14)
        plt.xlabel('Month', fontsize=12)
        plt.ylabel(f'Temperature ({display_units})', fontsize=12)
       
        # Ensure x-ticks are 1-12 and labeled correctly
        # Use the 'month' coordinate of plot_data for ticks if it's sensible (1-12)
        month_ticks = plot_data.coords.get('month', np.arange(1,13))
        if month_ticks.min() >= 1 and month_ticks.max() <=12 :
             plt.xticks(month_ticks.data, ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
                                     'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][int(month_ticks.min().item())-1 : int(month_ticks.max().item())])
        else: # Fallback if month coordinate is not 1-12
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a seasonal cycle (monthly average) line plot for 2m temperature.")
    ...
    plot_2m_temp_seasonal_cycle(ecmwf_file, nc_var_name, args.output)