...
def plot_precip_avg_heatmap(noaa_precip_file_path_str, nc_variable_name, output_path_str):
    ...
    try:
        ...
        display_units = data_var.attrs.get('units', 'mm') 
        cbar_label = f'Average Precipitation ({display_units})'
        plot_title = f'Average Precipitation' # Generic title, Streamlit provides context
        
        # Your custom blue colormap for precipitation was:
        # cmap_precip = LinearSegmentedColormap.from_list('precip_blues',
        #                                                 ['#FFFFFF', '#D1EEFC', '#75C6EF', '#1E90FF', '#0066CC', '#003366'])
        # Using a slightly expanded version for potentially better visual differentiation
        cmap_precip = LinearSegmentedColormap.from_list('precip_blues',
        ...
        if lat_coord_name not in ds_precip.coords or lon_coord_name not in ds_precip.coords:
            print(f"Error: Could not determine lat/lon coordinates. Found coords: {list(ds_precip.coords)}", file=sys.stderr)
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    # The function name from your original script was plot_noaa_precip_average,
    # but we made it more specific in the previous update.
