...
#     plot_ecmwf_data(ecmwf_file, var='msl', plot_type='timeseries')
...
def plot_msl_area_avg_timeseries(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Create an area average timeseries plot for Mean Sea Level Pressure (MSL).
    ...
    try:
        ...
        # Convert MSL from Pa to hPa if needed (heuristic: values > 10000 are likely Pa)
        if area_mean.max(skipna=True).item() > 10000 and ('pa' in display_units.lower() and 'hpa' not in display_units.lower()):
            area_mean = area_mean / 100  # Convert to hPa
            display_units = 'hPa'
            print(f"Data for '{nc_variable_name}' appears to be in Pa, converted to hPa.", file=sys.stdout)
        ...
        # Select color for msl (e.g., first color from a palette)
        plot_color = sns.color_palette("viridis", 5)[0] # Index 0 for 'msl'
        
        area_mean.plot(ax=plt.gca(), linewidth=1.5, color=plot_color) # Use plt.gca() to get current axes
       
        plt.title(f'Mean Sea Level Pressure (Area Average)', fontsize=14)
        plt.xlabel('Time', fontsize=12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate an area average timeseries plot for Mean Sea Level Pressure (MSL).")
    ...
    plot_msl_area_avg_timeseries(ecmwf_file, nc_var_name, args.output)