...
def plot_sst_avg_heatmap(noaa_sst_file_path_str, nc_variable_name, output_path_str):
    """
    Plot the time-averaged NOAA Sea Surface Temperature (SST) as a heatmap.
    ...
    try:
        ...
        # Calculate time average
        sst_avg = sst_data_var.mean(dim=dim_to_use_for_time, skipna=True)
        
        # Handle unit conversion for display
        display_units = "°C" # Target display unit for SST
        sst_avg_for_plot = sst_avg.copy() # Work on a copy for plotting

        if sst_avg.mean(skipna=True).item() > 100:  # Heuristic: if mean SST > 100, likely Kelvin
            sst_avg_for_plot = sst_avg - 273.15
            print(f"SST data for heatmap appears to be in Kelvin, converted to Celsius for plotting.", file=sys.stdout)
        else: # Assumed to be Celsius or already converted
            original_units = sst_data_var.attrs.get('units', '°C')
            if 'k' in original_units.lower(): # If units attribute explicitly says K but values are low
                display_units = "K" 
            print(f"SST data for heatmap assumed to be in Celsius or original units ({display_units}).", file=sys.stdout)

        cbar_label = f'Average Sea Surface Temperature ({display_units})'
        plot_title = f'Average Sea Surface Temperature'
        cmap = 'RdYlBu_r' # Common for temperature

        # Determine spatial coordinate names (typically 'lat', 'lon')
        lat_coord_name = 'lat' if 'lat' in ds_sst.coords else 'latitude'
        lon_coord_name = 'lon' if 'lon' in ds_sst.coords else 'longitude'

        if lat_coord_name not in ds_sst.coords or lon_coord_name not in ds_sst.coords:
            print(f"Error: Could not determine lat/lon coordinates. Looked for 'lat'/'latitude' and 'lon'/'longitude'. Found coords: {list(ds_sst.coords)}", file=sys.stderr)
            sys.exit(1)
            
        longitude_coords = ds_sst[lon_coord_name].values
        latitude_coords = ds_sst[lat_coord_name].values

        fig, ax = plt.subplots(figsize=(10, 8))
        sst_plot = ax.pcolormesh(longitude_coords, latitude_coords, sst_avg_for_plot.data,
                                   cmap=cmap, shading='auto')
       
        cbar = plt.colorbar(sst_plot, ax=ax, orientation='vertical', pad=0.05, aspect=30)
        ...
        try:
            start_date = pd.to_datetime(str(ds_sst[actual_time_coord_name].min().values)).strftime('%Y-%m-%d')
            end_date = pd.to_datetime(str(ds_sst[actual_time_coord_name].max().values)).strftime('%Y-%m-%d')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a time-averaged heatmap for NOAA SST.")
    ...
    plot_sst_avg_heatmap(noaa_file, nc_var_name, args.output)