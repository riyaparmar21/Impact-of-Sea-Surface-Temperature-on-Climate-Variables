...
#     plot_ecmwf_data(ecmwf_file, var='msl', plot_type='seasonal')
...
def plot_msl_seasonal_cycle(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Create a seasonal cycle plot (monthly averages over the year) for MSL.
    ...
    nc_variable_name : str
        The NetCDF variable name for MSL (e.g., 'msl').
    output_path_str : str
    ...
    try:
        ...
        if nc_variable_name not in ds:
            print(f"Error: Variable '{nc_variable_name}' (expected for MSL) not found in dataset {ecmwf_file_path}", file=sys.stderr)
        ...
        display_units = data_var.attrs.get('units', 'Pa') # Default to Pa if not specified
        
        # Convert MSL from Pa to hPa if needed
        if plot_data.max(skipna=True).item() > 10000 and ('pa' in display_units.lower() and 'hpa' not in display_units.lower()): # Heuristic for Pa
            plot_data = plot_data / 100  # Convert to hPa
            display_units = 'hPa'
            print(f"Data for '{nc_variable_name}' appears to be in Pa, converted to hPa for seasonal cycle plot.", file=sys.stdout)
        elif 'hpa' in display_units.lower():
            display_units = 'hPa' # Ensure consistency
        else:
            print(f"Data for '{nc_variable_name}' presented in original units: {display_units} (or already hPa-like values).", file=sys.stdout)
       
        # Select color for msl
        plot_color = sns.color_palette("viridis", 5)[0] # Index 0 for 'msl'
        ...
        plt.title(f'Mean Sea Level Pressure Seasonal Cycle (Area Average)', fontsize=14)
        plt.xlabel('Month', fontsize=12)
        plt.ylabel(f'Pressure ({display_units})', fontsize=12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a seasonal cycle (monthly average) line plot for MSL.")
    parser.add_argument('--files', required=True, nargs=1, help='Path to the input ECMWF NetCDF file.')
    parser.add_argument('--varnames', required=True, nargs=1, help="NetCDF variable name for MSL (e.g., 'msl').")
    ...
    plot_msl_seasonal_cycle(ecmwf_file, nc_var_name, args.output)