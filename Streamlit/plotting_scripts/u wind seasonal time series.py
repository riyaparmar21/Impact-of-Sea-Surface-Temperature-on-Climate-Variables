...
#     plot_ecmwf_data(ecmwf_file, var='u10', plot_type='seasonal')
...
def plot_u10_seasonal_cycle(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Create a seasonal cycle plot (monthly averages over the year) for area-averaged U10.
    ...
    nc_variable_name : str
        The NetCDF variable name for U10 (e.g., 'u10').
    output_path_str : str
    ...
    try:
        ...
        if nc_variable_name not in ds:
            print(f"Error: Variable '{nc_variable_name}' (expected for U10) not found in dataset {ecmwf_file_path}", file=sys.stderr)
        ...
        # Area average first
        u10_area_mean = data_var.mean(dim=[lat_coord_name, lon_coord_name], skipna=True)
        
        display_units = data_var.attrs.get('units', 'm/s') # Default to m/s for wind components
        
        # Group by month to create seasonal cycle plot using the area mean
        seasonal_cycle_data = u10_area_mean.groupby(f"{actual_time_coord_name}.month").mean(skipna=True)
        ...
        # Select color for U10
        # Original list: ['msl', 'tcc', 'u10', 'v10', 't2m'] -> u10 is index 2
        plot_color = sns.color_palette("viridis", 5)[2] 
        seasonal_cycle_data.plot(marker='o', color=plot_color)
       
        plt.title(f'10m U-Wind Component Seasonal Cycle (Area Average)', fontsize=14)
        plt.xlabel('Month', fontsize=12)
        plt.ylabel(f'U-Wind ({display_units})', fontsize=12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a seasonal cycle (monthly average) line plot for U10 wind.")
    parser.add_argument('--files', required=True, nargs=1, help='Path to the input ECMWF NetCDF file.')
    parser.add_argument('--varnames', required=True, nargs=1, help="NetCDF variable name for U10 (e.g., 'u10').")
    ...
    plot_u10_seasonal_cycle(ecmwf_file, nc_var_name, args.output)