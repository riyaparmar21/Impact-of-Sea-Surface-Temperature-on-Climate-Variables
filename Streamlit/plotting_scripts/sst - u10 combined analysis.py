...
def plot_sst_u10_combined_analysis(sst_file_path_str, sst_nc_var_name, 
                                   u10_file_path_str, u10_nc_var_name, 
                                   output_path_str):
    """
    Plots combined time series and scatter plot for SST and 10m U-Wind (U10), showing correlation.
    """
    try:
        ...
        series_u10 = load_and_average_variable(u10_file_path_str, u10_nc_var_name, 
                                               default_time_coord='valid_time', var_role="U10 Wind")
        if series_u10 is None: sys.exit(1)

        u10_display_units = ds_u10_var_for_units = xr.open_dataset(u10_file_path_str)[u10_nc_var_name].attrs.get('units', 'm/s') # Get units for label
        ...
        if not isinstance(series_u10.index, pd.DatetimeIndex):
            try: series_u10.index = pd.to_datetime(series_u10.index)
            except: print(f"Error: U10 series index could not be converted to DatetimeIndex.", file=sys.stderr); sys.exit(1)

        try:
            series_sst_aligned = series_sst.resample('ME').mean()
            series_u10_aligned = series_u10.resample('ME').mean()
            print("SST and U10 data resampled to monthly frequency for alignment.", file=sys.stdout)
        except Exception as e_resample:
            print(f"Warning: Could not resample SST/U10 to monthly: {e_resample}. Using original time resolution.", file=sys.stderr)
            series_sst_aligned = series_sst
            series_u10_aligned = series_u10

        common_df = pd.merge(
            series_sst_aligned.rename('SST'),
            series_u10_aligned.rename('U10'),
            left_index=True, right_index=True,
            how='inner'
        ).dropna()

        if common_df.empty or len(common_df) < 2:
            print("Error: No common overlapping data points found between SST and U10. Cannot compute correlation.", file=sys.stderr)
            fig, ax = plt.subplots(figsize=(12,6))
            ax.text(0.5,0.5, "Insufficient overlapping data for SST-U10 analysis", horizontalalignment='center', verticalalignment='center')
            ax.set_title("SST and 10m U-Wind Combined Analysis")
        else:
            correlation, p_value = stats.pearsonr(common_df['SST'], common_df['U10'])
            ...
            ax1_twin = ax1.twinx()
            ax1_twin.plot(common_df.index, common_df['U10'], 'r-', label=f'U10 Wind ({u10_display_units})')
            ax1_twin.set_ylabel(f'U10 Wind ({u10_display_units})', color='red', fontsize=12)
            ax1_twin.tick_params(axis='y', labelcolor='red')
            
            ax1.set_xlabel("Time", fontsize=12)
            ax1.set_title(f"SST and 10m U-Wind Time Series\nCorrelation: {correlation:.3f} (p-value: {p_value:.3e})", fontsize=14)
            ...
            ax2.scatter(common_df['SST'], common_df['U10'], alpha=0.6, color='darkorange') # Changed scatter color
            ax2.set_xlabel('SST (°C)', fontsize=12)
            ax2.set_ylabel(f'U10 Wind ({u10_display_units})', fontsize=12)
            ax2.set_title('SST vs. 10m U-Wind Scatter Plot with Trend Line', fontsize=12)
            
            z = np.polyfit(common_df['SST'], common_df['U10'], 1)
            p = np.poly1d(z)
        ...
        print(f"An unexpected error occurred in plot_sst_u10_combined_analysis: {e}", file=sys.stderr)
        sys.exit(1)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a combined analysis plot for SST and 10m U-Wind (U10).")
    parser.add_argument('--files', required=True, nargs='+', 
                        help='Paths to the input NetCDF files (SST and U10 sources).')
    ...
    parser.add_argument('--vars', required=True, nargs=2, # Expects exactly 2 internal keys
                        help="Two internal variable keys (e.g., 'noaa_sst' 'ecmwf_u10').")
    ...
    U10_KEY_IDENTIFIER_SUBSTRING = 'u10'

    SST_FILE_BASENAME_EXPECTED = "noaa_sst_masked.nc"
    U10_FILE_BASENAME_EXPECTED = "ecmwf datas.nc" # U10 is from ECMWF

    sst_file_arg, u10_file_arg = None, None
    sst_varname_arg, u10_varname_arg = None, None
    
    key1, key2 = args.vars[0], args.vars[1]
    varname1, varname2 = args.varnames[0], args.varnames[1]

    if SST_KEY_IDENTIFIER_SUBSTRING in key1.lower() and U10_KEY_IDENTIFIER_SUBSTRING in key2.lower():
        sst_varname_arg, u10_varname_arg = varname1, varname2
    elif U10_KEY_IDENTIFIER_SUBSTRING in key1.lower() and SST_KEY_IDENTIFIER_SUBSTRING in key2.lower():
        u10_varname_arg, sst_varname_arg = varname1, varname2
    else:
        print(f"Warning: Could not definitively identify SST and U10 roles from --vars: {args.vars}. Ensure one key contains '{SST_KEY_IDENTIFIER_SUBSTRING}' and the other '{U10_KEY_IDENTIFIER_SUBSTRING}'.", file=sys.stderr)
        sys.exit(1)

    for f_path_str in args.files:
        if Path(f_path_str).name == SST_FILE_BASENAME_EXPECTED:
            sst_file_arg = f_path_str
        if Path(f_path_str).name == U10_FILE_BASENAME_EXPECTED:
            u10_file_arg = f_path_str
            
    if not sst_file_arg and len(args.files) == 1:
        if Path(args.files[0]).name == SST_FILE_BASENAME_EXPECTED or SST_FILE_BASENAME_EXPECTED == U10_FILE_BASENAME_EXPECTED:
            sst_file_arg = args.files[0]
    
    if not u10_file_arg and len(args.files) == 1:
        if Path(args.files[0]).name == U10_FILE_BASENAME_EXPECTED or SST_FILE_BASENAME_EXPECTED == U10_FILE_BASENAME_EXPECTED:
            u10_file_arg = args.files[0]

    if not sst_file_arg or not u10_file_arg or not sst_varname_arg or not u10_varname_arg:
        print("Error: Could not determine necessary file paths or variable names for SST and U10.", file=sys.stderr)
        print(f"  SST File: {sst_file_arg}, SST Var: {sst_varname_arg}")
        print(f"  U10 File: {u10_file_arg}, U10 Var: {u10_varname_arg}")
        print(f"  Args.files: {args.files}, Args.vars: {args.vars}, Args.varnames: {args.varnames}")
        sys.exit(1)
    
    print(f"Plotting SST (file: '{sst_file_arg}', var: '{sst_varname_arg}') vs U10 (file: '{u10_file_arg}', var: '{u10_varname_arg}')", file=sys.stdout)

    plot_sst_u10_combined_analysis(sst_file_arg, sst_varname_arg, 
                                   u10_file_arg, u10_varname_arg, 
                                   args.output)