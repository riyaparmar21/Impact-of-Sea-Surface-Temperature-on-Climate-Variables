...
# fig_tcc_seasonal = plot_seasonal_averages(ds_ecmwf, 'tcc')
...
def plot_tcc_seasonal_avg_heatmaps(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Plot seasonal average heatmaps for Total Cloud Cover (TCC).
    ...
    try:
        ...
        # Unit conversion for TCC data before seasonal averaging
        display_units = data_var.attrs.get('units', 'fraction') # Default to fraction
        tcc_for_seasonal_avg = data_var.copy() 

        if data_var.max(skipna=True).item() <= 1.0 and data_var.min(skipna=True).item() >= 0.0 and display_units == 'fraction':
            tcc_for_seasonal_avg = data_var * 100
            display_units = '%' # Update units for label
            print(f"TCC data converted from fraction to percentage for seasonal averaging.", file=sys.stdout)
        elif display_units != '%':
             print(f"TCC data presented in original units: {display_units} for seasonal averaging.", file=sys.stdout)
        else: # Already in %
            display_units = '%'
        
        # Update the variable in ds_with_season if conversion happened
        ds_with_season[nc_variable_name] = tcc_for_seasonal_avg
        seasonal_avg_data_var = ds_with_season[nc_variable_name].groupby('season').mean(dim=dim_to_use_for_time, skipna=True)
        
        cmap = 'Blues' 
        label_text = 'Cloud Cover'
        ...
        for i, season_name in enumerate(season_order):
            ...
            if season_name in seasonal_avg_data_var.season.values:
                ...
                im = ax.pcolormesh(lon_coords.data, lat_coords.data, season_data_to_plot.data,
                                      cmap=cmap, shading='auto', vmin=0, vmax=100 if display_units == '%' else None) # Set vmin/vmax if %
        ...
        plt.suptitle(f'Seasonal Average Total Cloud Cover', fontsize=16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate seasonal average heatmaps for Total Cloud Cover (TCC).")
    ...
    plot_tcc_seasonal_avg_heatmaps(ecmwf_file, nc_var_name, args.output)