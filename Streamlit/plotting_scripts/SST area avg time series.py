...
def plot_sst_area_avg_timeseries(noaa_sst_file_path_str, nc_variable_name, output_path_str):
    """
    Create a timeseries plot for area-averaged NOAA SST.
    ...
    try:
        ...
        sst_mean = sst_data_var.mean(dim=[lat_coord_name, lon_coord_name], skipna=True)
        
        # Handle unit conversion for display
        display_units = "°C" # Target display unit
        if sst_mean.mean(skipna=True).item() > 100:  # Heuristic: if mean SST > 100, likely Kelvin
            sst_mean_for_plot = sst_mean - 273.15
            print(f"SST data appears to be in Kelvin, converted to Celsius for plotting.", file=sys.stdout)
        else: # Assumed to be Celsius or already converted
            sst_mean_for_plot = sst_mean
            ...
            if 'k' in original_units.lower(): # If units attribute explicitly says K but values are low
                display_units = "K" # Then display as K
            print(f"SST data assumed to be in Celsius or original units ({display_units}).", file=sys.stdout)
       
        plt.figure(figsize=(12, 6))
        sst_mean_for_plot.plot(linewidth=1.5, color='darkred')
       
        plt.title('Sea Surface Temperature (Area Average)', fontsize=14)
        plt.xlabel('Time', fontsize=12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a timeseries plot for area-averaged NOAA SST.")
    ...
    plot_sst_area_avg_timeseries(noaa_file, nc_var_name, args.output)