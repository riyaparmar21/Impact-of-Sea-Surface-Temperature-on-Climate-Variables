...
#     plot_ecmwf_data(ecmwf_file, var='v10', plot_type='seasonal')
...
def plot_v10_seasonal_cycle(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Create a seasonal cycle plot (monthly averages over the year) for area-averaged V10.
    ...
    try:
        ...
        # Area average first
        v10_area_mean = data_var.mean(dim=[lat_coord_name, lon_coord_name], skipna=True)
        ...
        seasonal_cycle_data = v10_area_mean.groupby(f"{actual_time_coord_name}.month").mean(skipna=True)
        ...
        plt.title(f'10m V-Wind Component Seasonal Cycle (Area Average)', fontsize=14)
        plt.xlabel('Month', fontsize=12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a seasonal cycle (monthly average) line plot for V10 wind.")
    ...
    plot_v10_seasonal_cycle(ecmwf_file, nc_var_name, args.output)