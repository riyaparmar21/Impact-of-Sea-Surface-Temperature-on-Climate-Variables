...
def plot_sst_seasonal_avg_heatmaps(noaa_sst_file_path_str, nc_variable_name, output_path_str):
    """
    Plot seasonal average heatmaps for NOAA Sea Surface Temperature (SST).
    ...
    try:
        ...
        ds_with_season = ds_sst.assign_coords(season=get_season(ds_sst[actual_time_coord_name], dim_to_use_for_time))
        
        # Handle potential unit conversion for SST data before seasonal averaging
        display_units = "°C" # Target display unit for SST
        sst_for_seasonal_avg = sst_data_var.copy() # Work on a copy

        if sst_data_var.mean(skipna=True).item() > 100:  # Heuristic: if mean SST > 100, likely Kelvin
            sst_for_seasonal_avg = sst_data_var - 273.15
            print(f"SST data for seasonal averaging appears to be in Kelvin, converted to Celsius.", file=sys.stdout)
        else: # Assumed to be Celsius or already converted
            original_units = sst_data_var.attrs.get('units', '°C')
            if 'k' in original_units.lower(): 
                display_units = "K" 
            print(f"SST data for seasonal averaging assumed to be in Celsius or original units ({display_units}).", file=sys.stdout)
        
        # Update the variable in ds_with_season if conversion happened
        ds_with_season[nc_variable_name] = sst_for_seasonal_avg
        ...
        if lat_coord_name not in ds_sst.coords or lon_coord_name not in ds_sst.coords:
            print(f"Error: Could not determine lat/lon coordinates. Found: {list(ds_sst.coords)}", file=sys.stderr)
            sys.exit(1)

        lon_coords = ds_sst[lon_coord_name]
        lat_coords = ds_sst[lat_coord_name]
        ...
        for i, season_name in enumerate(season_order):
            ...
            if season_name in seasonal_avg_data_var.season.values:
                ...
                im = ax.pcolormesh(lon_coords.data, lat_coords.data, season_data_to_plot.data,
                                      cmap=cmap, shading='auto') # vmin, vmax can be added if needed
        ...
        plt.suptitle(f'Seasonal Average Sea Surface Temperature', fontsize=16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate seasonal average heatmaps for NOAA SST.")
    ...
    plot_sst_seasonal_avg_heatmaps(noaa_file, nc_var_name, args.output)