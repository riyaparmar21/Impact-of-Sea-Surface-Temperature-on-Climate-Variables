...
def plot_msl_avg_heatmap(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Plot the time-averaged Mean Sea Level Pressure (MSL) as a heatmap.
    ...
    try:
        ...
        cbar_label = f'Pressure ({display_units})'
        plot_title = f'Average Mean Sea Level Pressure'
        cmap = 'viridis' # Default for MSL, can be changed
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a time-averaged heatmap for Mean Sea Level Pressure (MSL).")
    ...
    plot_msl_avg_heatmap(ecmwf_file, nc_var_name, args.output)