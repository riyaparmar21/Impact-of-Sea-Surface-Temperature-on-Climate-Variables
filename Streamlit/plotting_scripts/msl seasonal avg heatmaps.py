...
# fig_msl_seasonal = plot_seasonal_averages(ds_ecmwf, 'msl')
...
def plot_msl_seasonal_avg_heatmaps(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Plot seasonal average heatmaps for Mean Sea Level Pressure (MSL).
    ...
    try:
        ...
        # Unit conversion for MSL
        display_units = 'hPa' # Target display unit
        cmap = 'viridis' 
        label_text = 'Pressure'

        if not seasonal_avg_data_var.season.size == 0:
            sample_max_val = seasonal_avg_data_var.isel(season=0).max(skipna=True).item()
            current_units_attr = data_var.attrs.get('units', 'Pa').lower()
            if np.isfinite(sample_max_val) and sample_max_val > 10000 and 'pa' in current_units_attr and 'hpa' not in current_units_attr:
                seasonal_avg_data_var = seasonal_avg_data_var / 100
                print(f"Data for '{nc_variable_name}' appears to be in Pa, converted to hPa for seasonal plots.", file=sys.stdout)
            elif 'hpa' in current_units_attr:
                 pass # Already hPa
            else:
                display_units = data_var.attrs.get('units', 'hPa')
                print(f"Data for '{nc_variable_name}' presented in units: {display_units} (heuristic for Pa->hPa conversion not met or units already hPa).", file=sys.stdout)
        else:
        ...
        plt.suptitle(f'Seasonal Average Mean Sea Level Pressure', fontsize=16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate seasonal average heatmaps for Mean Sea Level Pressure (MSL).")
    ...
    plot_msl_seasonal_avg_heatmaps(ecmwf_file, nc_var_name, args.output)