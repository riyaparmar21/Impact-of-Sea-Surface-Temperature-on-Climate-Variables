...
def plot_sst_tcc_combined_analysis(sst_file_path_str, sst_nc_var_name, 
                                   tcc_file_path_str, tcc_nc_var_name, 
                                   output_path_str):
    """
    Plots combined time series and scatter plot for SST and Total Cloud Cover (TCC), showing correlation.
    """
    try:
        ...
        series_tcc = load_and_average_variable(tcc_file_path_str, tcc_nc_var_name, 
                                               default_time_coord='valid_time', var_role="TCC")
        if series_tcc is None: sys.exit(1)

        # Unit conversion for TCC (fraction to percentage if applicable)
        tcc_display_units = "%" # Target for TCC
        # TCC is often a fraction (0-1). Convert to percentage.
        if series_tcc.max() <= 1.0 and series_tcc.min() >= 0.0 :
            series_tcc = series_tcc * 100
            print(f"TCC data appears to be a fraction, converted to percentage.", file=sys.stdout)
        else: # If already looks like percentage or other scale, use as is but label as % or original
            original_units = ds[tcc_nc_var_name].attrs.get('units', '%') # Assume % if not specified
            tcc_display_units = original_units if original_units else '%'
        ...
        if not isinstance(series_tcc.index, pd.DatetimeIndex):
            try: series_tcc.index = pd.to_datetime(series_tcc.index)
            except: print(f"Error: TCC series index could not be converted to DatetimeIndex.", file=sys.stderr); sys.exit(1)

        try:
            series_sst_aligned = series_sst.resample('ME').mean()
            series_tcc_aligned = series_tcc.resample('ME').mean()
            print("SST and TCC data resampled to monthly frequency for alignment.", file=sys.stdout)
        except Exception as e_resample:
            print(f"Warning: Could not resample SST/TCC to monthly: {e_resample}. Using original time resolution.", file=sys.stderr)
            series_sst_aligned = series_sst
            series_tcc_aligned = series_tcc

        common_df = pd.merge(
            series_sst_aligned.rename('SST'),
            series_tcc_aligned.rename('TCC'),
            left_index=True, right_index=True,
            how='inner'
        ).dropna()

        if common_df.empty or len(common_df) < 2:
            print("Error: No common overlapping data points found between SST and TCC. Cannot compute correlation.", file=sys.stderr)
            fig, ax = plt.subplots(figsize=(12,6))
            ax.text(0.5,0.5, "Insufficient overlapping data for SST-TCC analysis", horizontalalignment='center', verticalalignment='center')
            ax.set_title("SST and Total Cloud Cover Combined Analysis")
        else:
            correlation, p_value = stats.pearsonr(common_df['SST'], common_df['TCC'])
            ...
            ax1_twin = ax1.twinx()
            ax1_twin.plot(common_df.index, common_df['TCC'], 'r-', label=f'TCC ({tcc_display_units})')
            ax1_twin.set_ylabel(f'TCC ({tcc_display_units})', color='red', fontsize=12)
            ax1_twin.tick_params(axis='y', labelcolor='red')
            
            ax1.set_xlabel("Time", fontsize=12)
            ax1.set_title(f"SST and Total Cloud Cover Time Series\nCorrelation: {correlation:.3f} (p-value: {p_value:.3e})", fontsize=14)
            ...
            ax2.scatter(common_df['SST'], common_df['TCC'], alpha=0.6, color='purple') # Changed scatter color
            ax2.set_xlabel('SST (°C)', fontsize=12)
            ax2.set_ylabel(f'TCC ({tcc_display_units})', fontsize=12)
            ax2.set_title('SST vs. Total Cloud Cover Scatter Plot with Trend Line', fontsize=12)
            
            z = np.polyfit(common_df['SST'], common_df['TCC'], 1)
            p = np.poly1d(z)
        ...
        print(f"An unexpected error occurred in plot_sst_tcc_combined_analysis: {e}", file=sys.stderr)
        sys.exit(1)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a combined analysis plot for SST and Total Cloud Cover (TCC).")
    ...
    TCC_FILE_BASENAME_EXPECTED = "ecmwf datas.nc" # TCC is from ECMWF
    ...
    if SST_KEY_IDENTIFIER_SUBSTRING in key1.lower() and TCC_KEY_IDENTIFIER_SUBSTRING in key2.lower():
        ...
        print(f"Warning: Could not definitively identify SST and TCC roles from --vars: {args.vars}. Ensure one key contains '{SST_KEY_IDENTIFIER_SUBSTRING}' and the other '{TCC_KEY_IDENTIFIER_SUBSTRING}'.", file=sys.stderr)
        sys.exit(1)

    for f_path_str in args.files:
        if Path(f_path_str).name == SST_FILE_BASENAME_EXPECTED:
            sst_file_arg = f_path_str
        if Path(f_path_str).name == TCC_FILE_BASENAME_EXPECTED:
            tcc_file_arg = f_path_str
            
    if not sst_file_arg and len(args.files) == 1:
        if Path(args.files[0]).name == SST_FILE_BASENAME_EXPECTED or SST_FILE_BASENAME_EXPECTED == TCC_FILE_BASENAME_EXPECTED:
            sst_file_arg = args.files[0]
    
    if not tcc_file_arg and len(args.files) == 1:
        if Path(args.files[0]).name == TCC_FILE_BASENAME_EXPECTED or SST_FILE_BASENAME_EXPECTED == TCC_FILE_BASENAME_EXPECTED:
            tcc_file_arg = args.files[0]

    if not sst_file_arg or not tcc_file_arg or not sst_varname_arg or not tcc_varname_arg:
        print("Error: Could not determine necessary file paths or variable names for SST and TCC.", file=sys.stderr)
        print(f"  SST File: {sst_file_arg}, SST Var: {sst_varname_arg}")
        print(f"  TCC File: {tcc_file_arg}, TCC Var: {tcc_varname_arg}")
        print(f"  Args.files: {args.files}, Args.vars: {args.vars}, Args.varnames: {args.varnames}")
        sys.exit(1)
    
    print(f"Plotting SST (file: '{sst_file_arg}', var: '{sst_varname_arg}') vs TCC (file: '{tcc_file_arg}', var: '{tcc_varname_arg}')", file=sys.stdout)

    plot_sst_tcc_combined_analysis(sst_file_arg, sst_varname_arg, 
