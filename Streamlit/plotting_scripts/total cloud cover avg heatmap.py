...
def plot_tcc_avg_heatmap(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Plot the time-averaged Total Cloud Cover (TCC) as a heatmap.
    ...
    try:
        ...
        # Unit handling for TCC
        display_units = data_var.attrs.get('units', 'fraction') # Default to fraction
        var_avg_for_plot = var_avg.copy()

        if var_avg.max(skipna=True).item() <= 1.0 and var_avg.min(skipna=True).item() >= 0.0 and display_units == 'fraction':
            var_avg_for_plot = var_avg * 100
        ...
        cbar_label = f'Cloud Cover ({display_units})'
        plot_title = f'Average Total Cloud Cover'
        cmap = 'Blues' # Appropriate for cloud cover
        ...
        fig, ax = plt.subplots(figsize=(10, 8))
        var_plot = ax.pcolormesh(longitude_coords, latitude_coords, var_avg_for_plot.data,
                                cmap=cmap, shading='auto')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a time-averaged heatmap for Total Cloud Cover (TCC).")
    ...
    plot_tcc_avg_heatmap(ecmwf_file, nc_var_name, args.output)