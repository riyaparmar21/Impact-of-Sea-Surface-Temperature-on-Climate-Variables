...
#     plot_ecmwf_data(ecmwf_file, var='tcc', plot_type='timeseries')
...
def plot_tcc_area_avg_timeseries(ecmwf_file_path_str, nc_variable_name, output_path_str):
    """
    Create an area average timeseries plot for Total Cloud Cover (TCC).
    ...
    try:
        ...
        # Handle unit conversion for TCC (fraction 0-1 to percentage 0-100)
        display_units = data_var.attrs.get('units', 'fraction') # Default to fraction
        area_mean_for_plot = area_mean.copy()

        if area_mean.max(skipna=True).item() <= 1.0 and area_mean.min(skipna=True).item() >= 0.0 and display_units == 'fraction':
            area_mean_for_plot = area_mean * 100
            display_units = '%' # Update units for label
            print(f"TCC data converted from fraction to percentage for plotting.", file=sys.stdout)
        elif display_units != '%': # If not fraction and not already %, keep original
            print(f"TCC data presented in original units: {display_units} for plotting.", file=sys.stdout)
        else: # Already in %
            display_units = '%'
        ...
        # Select color for TCC (e.g., second color from a palette)
        plot_color = sns.color_palette("viridis", 5)[1] # Index 1 for 'tcc'
        
        area_mean_for_plot.plot(ax=plt.gca(), linewidth=1.5, color=plot_color)
       
        plt.title(f'Total Cloud Cover (Area Average)', fontsize=14)
        plt.xlabel('Time', fontsize=12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate an area average timeseries plot for Total Cloud Cover (TCC).")
    ...
    plot_tcc_area_avg_timeseries(ecmwf_file, nc_var_name, args.output)